...
// Navigation par page
function page($num_total, $page, $filter = array())
{
	global $num_pp, $res;
...
function input($key = null, $filter = null)
{
	...
	if(@$filter['name']) echo' name="'.$filter['name'].'"';
...
function tag($key = null, $filter = array())
{
	...
	.'>';
		...
		$i = 1;
		$sel_tag = $GLOBALS['connect']->query("SELECT * FROM ".$GLOBALS['table_tag']." WHERE id='".(int)$GLOBALS['id']."' AND zone='".$key."' AND lang='".$GLOBALS['lang']."' ORDER BY ordre ASC LIMIT 20");
