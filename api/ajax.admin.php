...
switch($_GET['mode'])
{
	...
	case "add-content":// Dialog pour ajouter une page
		...
		// Si on a des droits d'ajout de contenu, on affiche la dialog
		if(stripos(json_encode($_SESSION['auth']),'add-') !== false)
		{
			?>
			<link rel="stylesheet" href="<?=$GLOBALS['jquery_ui_css']?>">
	...
	case "update":// Sauvegarde du contenu éditable de la page
		...
		if(isset($_POST['nav']))
		{
			...
			// Insert ou update ?
			if(isset($res_nav['type'])) $sql = "UPDATE"; else $sql = "INSERT INTO";
			$sql .= " ".$table_meta." SET ";
			$sql .= "id = '0', ";
			$sql .= "type = 'nav', ";
			$sql .= "cle = '".$lang."', ";
			$sql .= "val = '".addslashes($json_nav)."' ";
			if(isset($res_nav['type'])) $sql .= "WHERE type='nav' AND cle='".$lang."' LIMIT 1";
		...
		if(isset($_POST['header']))
		{
			...
			// Insert ou update ?
			if(isset($res_header['type'])) $sql = "UPDATE"; else $sql = "INSERT INTO";
			$sql .= " ".$table_meta." SET ";
			$sql .= "id = '0', ";
			$sql .= "type = 'header', ";
			$sql .= "cle = '".$lang."', ";
			$sql .= "val = '".addslashes($json_header)."' ";
			if(isset($res_header['type'])) $sql .= "WHERE type='header' AND cle='".$lang."' LIMIT 1";
		...
		if(isset($_POST['footer']))
		{
			...
			// Insert ou update ?
			if(isset($res_footer['type'])) $sql = "UPDATE"; else $sql = "INSERT INTO";
			$sql .= " ".$table_meta." SET ";
			$sql .= "id = '0', ";
			$sql .= "type = 'footer', ";
			$sql .= "cle = '".$lang."', ";
			$sql .= "val = '".addslashes($json_footer)."' ";
			if(isset($res_footer['type'])) $sql .= "WHERE type='footer' AND cle='".$lang."' LIMIT 1";
	...
	case "tags":// Liste les tags pour l'auto-complete
		...
		if(isset($tab_tag)) echo json_encode($tab_tag);//JSON_UNESCAPED_UNICODE
