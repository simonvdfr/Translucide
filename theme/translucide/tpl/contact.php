...
if($_POST["email"] and $_POST["message"] and isset($_POST["question"]) and !$_POST["champ_vide"])// champ_vide pour eviter les bot qui remplisse tous les champs
{
	...
	<section class="mw960p mod center mtl">
		<article class="fl w70 prl pbm mbm tofadein">			
			...
			<form id="contact" class="mat">
				...
				<div>
					<label class="bold" for="email"><?_e("Email")?> :</label> <input type="email" name="email" id="email" required placeholder="exemple@mymail.com" class="w40 vatt">
				</div>
				
				<div>
					<label class="bold" for="message"><?_e("Message")?> :</label> <textarea name="message" id="message" required class="w100 mbt" style="height: 200px;"></textarea>
				</div>
				
				<div class="">
					<label class="bold" for="question"><?_e("Question")?> : <?=($nb1." ".$operator." ".$nb2);?> = </label> <input type="text" name="question" id="question" required class="w50p vatt">
				</div>
