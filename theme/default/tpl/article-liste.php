...
while($res_fiche = $sel_fiche->fetch_assoc()) {
    ...
    ?>
		<article class="mod plm mrm mtl mbm">
			...
			<h2 class="mts up bigger"><a href="<?=make_url($res_fiche['url'], ["domaine" => true]);?>" class="tdn"><?=$res_fiche['title']?></a><?=$state?></h2>

			<?php if(isset($content_fiche['texte'])) {
			    echo word_cut($content_fiche['texte'], '350')."...";
			}?>

			<div class="fr mtm"><a href="<?=make_url($res_fiche['url'], ["domaine" => true]);?>" class="bt bg-color bold"><?php _e("Lire l'article")?></a></div>
